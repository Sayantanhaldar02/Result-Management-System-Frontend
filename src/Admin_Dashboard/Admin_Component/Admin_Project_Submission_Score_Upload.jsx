...
import { useUploadproject_submissionMutation, useUpdateproject_submissionMutation, useDeleteproject_submissionMutation, useGetAllproject_submissionQuery } from '../../Api/Project_Submission_api/Project_Submission_api';
...
const Admin_Project_Submission_Score_Upload = ({ value, index }) => {
    ...
    const handleClickUpdateFormOpen = (updateDetails) => {
        ...
        setStudentDetails({
            ...
            projectTitle: updateDetails.projectTitle,
            submissionDate: updateDetails.submissionDate,
            score: updateDetails.score,
            feedback: updateDetails.feedback,
    ...
    const [updateData, UpdateResponse] = useUpdateproject_submissionMutation()
    // Update Handeler
    ...
    const [uploadFile, UploadResponse] = useUploadproject_submissionMutation()
    // submit handeler
    const handelUploadFile = async () => {
        ...
        formData.append("projectSubmissionFile", file);
    ...
    const { data: fetchData, isLoading, isError, error, isSuccess, isFetching, refetch } = useGetAllproject_submissionQuery({ page, limit });

    useEffect(() => {
        if (isSuccess && fetchData && fetchData.data) {
            setAllData((prevItems) => {
                // Only append new items if they're not already in the list
                const newItems = fetchData.data.filter((item) => !prevItems.some((prevItem) => prevItem._id === item._id));
                return page === 1 ? fetchData.data : [...prevItems, ...newItems];
            });
        }
    }, [isSuccess, fetchData, page]);
    ...
    const handleScroll = useCallback(() => {
        ...
        if (scrollPosition >= maxScrollPosition && !isFetching && page < fetchData?.pagination?.totalPages) {
            setPage((prev) => prev + 1); // Load the next page
        }
    }, [isFetching, page, fetchData]);
    ...
    const [deleteRecord, deleteResponse] = useDeleteproject_submissionMutation();
    const handelDeleteAssmentRecored = async () => {
        ...
        <>
            <CustomTabPanel value={value} index={index}>
                <div className='my-[30px]'>
                    <div className='w-[100%] flex justify-between items-center py-4 px-2'>
                        <p className='text-[20px] font-semibold'>Project Submission Score Upload</p>
                    ...
                    <div className='w-[100%] mx-auto border-2 border-gray-600 '>
                        <TableContainer component={Paper} sx={{ maxHeight: 440 }} onScroll={handleScroll} >
                            <Table stickyHeader aria-label="simple table" className='rounded-lg'>
                                ...
                                <TableBody>
                                    {allData && allData.length > 0 && allData.map((row, index) => (
                                        <TableRow
                                            key={row._id}
                                        ...
                                        >
                                            ...
                                            <TableCell align="center">{new Date(row.submissionDate).toLocaleDateString()}</TableCell>
            ...
            >
                <DialogTitle sx={{ m: 0, p: 2 }} id="customized-dialog-title">
                    Project Submission Score Upload
                </DialogTitle>
                <IconButton
                    aria-label="close"
                    onClick={handleClose}
            ...
            >
                ...
                <DialogContent>
                    <form className='w-full flex flex-col gap-4' >
                        ...
                        <TextField name='projectTitle' value={studentDetails.projectTitle} onChange={onChnageHandeler} id="outlined-basic" label="Project Title" variant="outlined" className='w-[100%] my-2' />
                        <TextField name='submissionDate' type='date' value={studentDetails.submissionDate && new Date(studentDetails.submissionDate).toISOString().split("T")[0]} onChange={onChnageHandeler} id="outlined-basic" label="Submission Date" variant="outlined" className='w-[100%] my-2' />
                        ...
                        <TextField name='feedback' value={studentDetails.feedback} onChange={onChnageHandeler} label="Feedback" id="outlined-basic" variant="outlined" className='w-[100%] my-2' />
            ...
            >
                ...
                <DialogActions>
                    <button onClick={handelDeleteAssmentRecored} className='border-2 border-gray-600 px-4 py-2 font-semibold text-white bg-gray-600 rounded-[5px] w-full'>Delete</button>
                </DialogActions>
            </Dialog>
            {/* delete dialog end */}
        </>
...
export default Admin_Project_Submission_Score_Upload